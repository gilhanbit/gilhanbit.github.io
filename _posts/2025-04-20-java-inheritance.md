...
| 2 | **“부모 클래스의 기능이 자식에게 의미가 있는가?”**<br>→ 상속받은 메서드들이 실제로 유용하게 쓰이는가? | ✔️ / ❌ |
| 3 | **“공통 기능이 여러 클래스에 중복되어 작성되고 있는가?”**<br>→ 반복되는 코드 제거 목적이라면 상속 고려 | ✔️ / ❌ |
| 4 | **“부모 클래스가 자식 클래스보다 더 일반적인 개념인가?”**<br>→ 추상적이고 범용적인 기능을 포함하는가? | ✔️ / ❌ |
| 5 | **“부모 클래스는 자식과 독립적으로 잘 설계되어 있는가?”**<br>→ 자식이 부모의 내부 구현에 너무 의존하지 않는가? | ✔️ / ❌ |
| 6 | **“단일 상속만으로 충분한가?”**<br>→ 여러 상위 클래스를 동시에 쓰고 싶다면 인터페이스나 구성 고려 | ✔️ / ❌ |
| 7 | **“자식 클래스가 부모 클래스의 기능을 바꾸거나 일부만 사용할 필요가 없는가?”**<br>→ 필요 없는 기능까지 강제로 받지 않는가? | ✔️ / ❌ |
