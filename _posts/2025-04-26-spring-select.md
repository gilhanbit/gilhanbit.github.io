...
4번 mapper의 파라미터를 xml에 보낼 때 사용하는 @Param을 파라미터마다 일일이 설정해줘야 한다.

만약 몇 십개가 되는 항목이 있다면?

일일이 써줘야 한다고 한다.

생각했던 것보다 비효율적인 부분이 많구나 생각했지만, 효율보다 안정성, 유지보수의 용이성을 더 크게 생각하고 중요하게 하기 때문인 것 같다.

, . ; : 알파벳 등등 정말 작은 오타로, 위치가 틀렸다는 이유로 서비스가 동작이 안될 수 있기 때문이다.
